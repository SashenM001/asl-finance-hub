...
**Disabled pages:** `-_app.budget.tsx` (Budget vs. actual), `-_app.performance.tsx` (Performance metrics), and `-_app.review.tsx` (Monthly pass/fail tracker) exist in the codebase but are **disabled** — their filenames are prefixed with `-` so TanStack Router does not register them as routes, and their sidebar nav links in `AppShell.tsx` are commented out. Their tables (`budget_actual`, `monthly_review`) still exist and the data layer still carries them, but the EFB has not yet supplied data for these pages. To re-enable a page, drop the `-` prefix and uncomment its nav entry.
...
Financial data is broken down by AIESEC function codes, defined as the `FunctionCode` type and `FUNCTION_CODES` array in `src/lib/finance.ts` (with matching `FUNCTION_COLORS`): `iGV`, `iGT`, `oGV`, `oGT`, `ELD`, `EwA`, `Miscellaneous`, `NMF`, `Conference`, `National Conference Delegation`, `Project Management`. These appear throughout the metrics tables and chart components — use the exported constants rather than hardcoding the list.

> ⚠️ **Enum drift:** the live Supabase `function_code` enum (see `src/integrations/supabase/types.ts`, generated via `supabase gen types`) has 16 values: `iGV, iGT, oGV, oGT, ELD, EwA, BD, iGTa, iGTe, oGTa, oGTe, Conference, NMF, Miscellaneous, National Conference Delegation, Project Management`. The extra `BD, iGTa, iGTe, oGTa, oGTe` values are legacy/unused leftovers from the original migration (`20260424043656_…sql`, which only had 7 values: `iGV, iGT, oGV, oGT, ELD, EwA, BD`) — the frontend's 11-value list above is the actual set in use. `Project Management` was added 2026-07-01 via migration `20260701000000_add_project_management_function_code.sql` to split Project Management income/costs out of `Miscellaneous` (previously GFB codes `7305`-`7309`/`7903`-`7905` were misclassified as `Miscellaneous`; `7301`-`7304`/`7901`-`7902`, the conference-specific PM lines, were already correctly mapped to `Conference`). Treat `src/lib/finance.ts` as the source of truth for which codes the UI actually uses, and `types.ts` as the source of truth for the current live DB enum.

### Google Sheets Sync

External financial data flows in from Google Sheets. Financial and audit data each have their own fully-separated two-step sync (no shared Edge Function anymore):

- **Financial** — driven by the `useSheetSync` hook (`src/hooks/useSheetSync.ts`):
  1. **Trigger** — POST to the `trigger-financial-sync` Edge Function, which (after verifying the JWT + `mc_user` role) calls an external AppScript webhook with `sync: "financial"` that consolidates the origin sheets into the `MASTER_COMBINED_TALL` master tab. Webhook URL + secret live as server-side Supabase secrets and never reach the browser.
  2. **Pull** — the browser calls `fetchFinancialData()` (`client.ts`), which POSTs to the `pull-financial-data` Edge Function (with the user's Supabase JWT), authenticates to Google as a Service Account, reads `MASTER_COMBINED_TALL` from the **private** master spreadsheet, and returns the rows. `syncSheetData()` (`src/integrations/googleSheets/sync.ts`) then aggregates per `(entity, month)` and upserts into `monthly_metrics`/`revenue_streams`/`cost_breakdown`.
- **Audit** — driven by the `useAuditSync` hook (`src/hooks/useAuditSync.ts`), same pattern via `trigger-audit-sync` → AppScript (`sync: "audit"`) → `MASTER_AUDIT_TALL` → `pull-audit-data` → `fetchAuditData()` → `syncAuditData()` (`src/integrations/googleSheets/auditSync.ts`) → upserts into `audit_scores`.

The SA key (`GOOGLE_SA_KEY`) and AppScript webhook secrets (`APPSCRIPT_WEBHOOK_URL`, `APPSCRIPT_SECRET`) are Supabase secrets shared across the financial/audit function pairs and never reach the browser. The four Edge Functions intentionally duplicate their auth/SA-token logic rather than share code (a "full-separation" design — see comments in each `index.ts`).

Supporting files: `client.ts` (`fetchFinancialData()` / `fetchAuditData()` route through their respective `pull-*` Edge Function), `mapper.ts` (`parseRow` → exact `GFB_DICTIONARY` lookup → classify GFB codes / function code / normalize), `index.ts` (public exports). The AppScript that builds both master tabs (routed by `sync` in a single `doPost`) is mirrored at `appscript/master-combined-tall-sync.gs` and `appscript/master-audit-tall-sync.gs`. `VITE_GOOGLE_SHEETS_API_KEY` is **no longer used for the financial/audit sync**; it remains in `client.ts` only for `fetchSheetDataMultiple` / `getSheetMetadata` (not called by the active sync path).
...
- `20260629000000_add_sync_upsert_constraints.sql` — adds the unique constraints the sheet-sync upsert relies on.
- `20260701000000_add_project_management_function_code.sql` — adds the `'Project Management'` value to the `function_code` enum.

**Known drift between the original migration and the live DB:**
- `function_code` enum: original file has 7 values (`iGV, iGT, oGV, oGT, ELD, EwA, BD`); the live DB now has 16 values (see `src/integrations/supabase/types.ts`, which is the authoritative generated reflection of the live enum — regenerate it with `npx supabase gen types typescript --linked > src/integrations/supabase/types.ts` after any enum-changing migration).
- The pulled migration file (`20260625175845_remote_schema.sql`) plus the two migrations after it reflect the live state — trust `types.ts` over any single migration file for the current schema.
