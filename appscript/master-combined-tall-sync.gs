...
//Verbose logged doPost for debugging (WARNING: RETUNRS ALL REQ PARAMS INCLUDING SECRETS. IF USING THIS FUNCTION, ROTATE WEBHOOK SECRET)
// function doPost(e) {
//   var dbg = {};
//   try {
//     var params = JSON.parse(e.postData.contents);
//     dbg.params = params;                       // what did the Edge Function actually send?
//     if (!WEBHOOK_SECRET || params.secret !== WEBHOOK_SECRET) {
//       return ContentService.createTextOutput(
//         JSON.stringify({ ok: false, error: "Unauthorized" })
//       ).setMimeType(ContentService.MimeType.JSON);
...
//       );
//     }
//     dbg.resultType = typeof result;            // "object" or "undefined"?

//     return ContentService.createTextOutput(
//       JSON.stringify({ ok: true, _v: "v3", rowsWritten: result.rowsWritten, warnings: result.warnings })
//     ).setMimeType(ContentService.MimeType.JSON);
//   } catch (err) {
//     return ContentService.createTextOutput(
//       JSON.stringify({ ok: false, _v: "v3", error: err.message, stack: String(err.stack), dbg: dbg })
//     ).setMimeType(ContentService.MimeType.JSON);
//   }
// }
...
function syncCombinedTallMasterSheet(mode, filterTerm, filterMonth) {
  ...
  if (mode === "term" && filterTerm) {
    sheetsToProcess = CONSOLIDATED_SHEETS.filter(function (c) {
      return c.term === filterTerm;
    });
    ...
    try {
      ...
      ss.getSheets().forEach(function (sheet) {
        try {
          ...
          if (tabClean.indexOf("PNL") === 0 || tabClean.indexOf("[PNL]") === 0) {
            ...
            if (lcName.toUpperCase() !== "CONSOLIDATED") {
              ...
              allRows = allRows.concat(
                _extractTallRows(sheet, lcName, config.term, "PnL", filterMonth),
              );
              foundValidTabs = true;
            }
            ...
            if (lcName.toUpperCase() !== "CONSOLIDATED") {
              ...
              allRows = allRows.concat(
                _extractTallRows(sheet, lcName, config.term, "CFS", filterMonth),
              );
              foundValidTabs = true;
            }
          }
          ...
          warnings.push(
            "Term " + config.term + " — skipped tab '" + sheet.getName() + "': " + tabError.message,
          );
        }
      });
...
function _extractTallRows(sheet, lc, term, reportType, filterMonth) {
  ...
  for (var i = 0; i < data.length; i++) {
    if (
      String(data[i][0]).trim().toUpperCase() === "GFB CODE" ||
