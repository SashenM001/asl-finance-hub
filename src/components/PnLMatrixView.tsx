...
const ROW_LABELS = [
  ...
  { id: "oGT income", label: "oGT income", type: "currency", indent: false },
  {
    ...
    label: "ELD income",
    type: "currency",
    style: "font-semibold bg-slate-50/80 text-slate-800",
    stickyStyle: "font-semibold bg-slate-50/80 text-slate-800",
  },
  ...
  {
    id: "Gross Profit/Loss",
    label: "Gross Profit/Loss",
    type: "currency",
    style: "font-bold bg-slate-100 text-slate-900 border-b-2",
    stickyStyle: "font-bold bg-slate-100 text-slate-900 border-b-2",
  },
  ...
  {
    id: "Project management income",
    label: "Project management income",
    type: "currency",
    indent: false,
  },
  { id: "Miscellaneous income", label: "Miscellaneous income", type: "currency", indent: false },
  {
    ...
    label: "Total income",
    type: "currency",
    style: "font-bold bg-slate-200 text-slate-900 border-b-2",
    stickyStyle: "font-bold bg-slate-200 text-slate-900 border-b-2",
  },
  ...
  {
    id: "National conference expenses",
    label: "National conference expenses",
    type: "currency",
    indent: false,
  },
  { id: "NMF expenses", label: "NMF expenses", type: "currency", indent: false },
  {
    id: "Project management expenses",
...

  {
    ...
    type: "currency",
    style: "font-extrabold bg-slate-800 text-white hover:bg-slate-700 text-xl",
    stickyStyle: "font-extrabold bg-slate-800 text-white text-xl",
  },
];

export function PnLMatrixView({
  configs,
  onAddConfig,
  onRemoveConfig,
  onEditConfig,
  ...
  const [rawData, setRawData] = useState<
    Record<string, { configHash: string; revenue: any[]; costs: any[] }>
  >({});
    ...
    const fetchAllData = async () => {
      ...
      await Promise.all(
        configs.map(async (config) => {
          ...
          const entityIds =
            effectiveEntityId && effectiveEntityId !== "Select LC" && effectiveEntityId !== "all"
          ...
          let revQ = supabase
            ...
            .select("entity_id,period_month,function_code,amount");
          ...
          if (config.function && config.function !== "all")
            revQ = revQ.eq("function_code", config.function);
          ...
          if (config.function && config.function !== "all")
            costQ = costQ.eq("function_code", config.function);
  ...
  const calculatedData = useMemo(() => {
    ...
    configs.forEach((config) => {
      ...
      const sumRev = (fn: string) =>
        revenue
          .filter((r) => r.function_code === fn)
          .reduce((acc, curr) => acc + Number(curr.amount), 0);
      const sumCost = (fn: string) =>
        costs
          .filter((c) => c.function_code === fn)
          .reduce((acc, curr) => acc + Number(curr.amount), 0);
      ...
      const pmIncome = sumRev("Project Management");
      ...
      const pmExpenses = sumCost("Project Management");
          ...
          <table className="w-full text-sm text-left whitespace-nowrap">
            <thead className="bg-slate-50 text-slate-600 border-b border-slate-200">
              <tr>
                ...
                {/* Dynamic Columns */}
                {configs.map((config) => {
                  const pinnedConfigsInOrder = configs.filter((c) =>
                    pinnedConfigIds.includes(c.id),
                  );
                  ...
                  return (
                    <th
                      id={`card-${config.id}`}
                      key={config.id}
                      style={isPinned ? { left: `${220 + pinnedIndex * 200}px` } : {}}
                      className={`p-4 min-w-[200px] w-[200px] align-top group transition-all duration-200 ${isPinned ? "sticky z-20 bg-blue-50/90 backdrop-blur-sm border-r-2 border-blue-200 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]" : "border-r border-slate-200 bg-white relative shadow-[0_1px_0_0_#e2e8f0]"}`}
                    >
                      <div className="flex flex-col gap-2">
                        <div className="flex items-start justify-between">
                            ...
                            >
                              {lockEntity ? (
                                <option value={profile?.entity_id || ""}>
                                  {entities.find((e) => e.id === profile?.entity_id)?.name
                                    ? formatEntityName(
                                        entities.find((e) => e.id === profile?.entity_id)!.name,
                                      )
                                    : "Your LC"}
                          ...
                          <div
                            className={`flex items-center gap-1 transition-opacity shrink-0 ${isPinned ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}
                          >
                            <Button
                              variant="ghost"
                              size="icon"
                        ...
                        <div className="flex flex-col gap-1 w-full mt-1">
                          <div className="flex items-center gap-1 justify-end">
                            ...
                            <input
                              ...
                              value={config.from || ""}
                              onChange={(e) =>
                                onUpdateConfig(config.id, { from: e.target.value, term: "Custom" })
                              }
                          ...
                          <div className="flex items-center gap-1 justify-end">
                            ...
                            <input
                              ...
                              value={config.to || ""}
                              onChange={(e) =>
                                onUpdateConfig(config.id, { to: e.target.value, term: "Custom" })
                              }
                              className="text-[10px] p-0.5 bg-transparent border-b border-gray-300 w-full focus:outline-none focus:border-blue-500 text-slate-700 text-right"
                            />
                          </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {ROW_LABELS.map((row) => {
                ...
                const stickyStyle =
                  row.stickyStyle || "bg-white group-hover:bg-[#f4f7fa] text-slate-700";

                return (
                  <tr key={row.id} className={`group ${rowStyle} transition-colors`}>
                    {/* Row Label, Sticky */}
                    <td
                      className={`sticky left-0 z-30 ${stickyStyle} min-w-[220px] w-[220px] ${row.indent ? "pl-8" : "pl-4"} pr-4 py-3 border-r border-slate-200 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] transition-colors`}
                    >
                      {row.label}
                    </td>
                    ...
                    {configs.map((config) => {
                      ...
                      return (
                        <td
                          key={`${config.id}-${row.id}`}
                          style={isPinned ? { left: `${220 + pinnedIndex * 200}px` } : {}}
                          className={`px-4 py-3 text-right font-mono text-sm ${row.style ? "text-inherit" : "text-slate-600"} transition-all duration-200 min-w-[200px] w-[200px] ${isPinned ? "sticky z-20 bg-blue-50/90 backdrop-blur-sm border-r-2 border-blue-200 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]" : "border-r border-slate-100"}`}
                        >
